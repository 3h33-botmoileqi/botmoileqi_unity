...
namespace CrazyMinnow.AmplitudeWebGL
{
    ...
    public class Amplitude : MonoBehaviour
    {
        ...
        [HideInInspector]
        public enum DataType { Amplitude, Frequency}
        [HideInInspector]
        public DataType dataType = DataType.Amplitude;
        
        [HideInInspector]
        public string[] sampleSizeNames = new string[] { "32", "64", "128", "256", "512", "1024", "2048", "4096" };
        [HideInInspector]
        public int[] sampleSizeVals = new int[] { 32, 64, 128, 256, 512, 1024, 2048, 4096 };
        ...
        [HideInInspector]
        private const float freqBoostMin = 4f; // Actually negative max, but we'll use positive to make scaling easier
        [HideInInspector]
        private const float freqBoostMax = 40f; // Actually negative min, but we'll use positive to make scaling easier
        ...
        {            
            ...
            {   
                ...
                if (audioSource.isPlaying)
                {
                    ...
                    {
                        if (sample.Length == 32) sample = new float[64];
